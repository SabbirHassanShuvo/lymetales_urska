...
{{-- Policy Content --}}
<div class="card">
    <div class="card-title"><span class="dot" style="background:#6366f1"></span> Policy Content</div>
    <div>
        <label class="label">Page Body Content</label>
        <textarea id="policy_body" name="sections_body" rows="15" class="input" style="resize:vertical">{!! $combinedBody !!}</textarea>
    </div>
</div>
...
<script>
    let policyEditor;
    ClassicEditor.create(document.querySelector('#policy_body')).then(editor => {
        policyEditor = editor;
    }).catch(err => console.error(err));

    document.querySelector('form').addEventListener('submit', function () {
        if (policyEditor) {
            document.querySelector('#policy_body').value = policyEditor.getData();
        }
    });
</script>
<style>
