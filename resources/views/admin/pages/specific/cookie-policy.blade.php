...
<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.75rem;margin-bottom:1.5rem">
    <div>
        ...
        <h2 style="font-size:1.5rem;font-weight:800;color:#111827;margin:0">Edit Cookie Policy</h2>
        <p style="font-size:0.85rem;color:#9ca3af;margin-top:0.25rem">Manage the Cookie Policy sections</p>
    </div>
...
{{-- Policy Content --}}
<div class="card">
    <div class="card-title"><span class="dot" style="background:#6366f1"></span> Cookie Policy Content</div>
    <div>
        <label class="label">Page Body Content</label>
        <textarea id="cookie_policy_body" name="sections_body" rows="15" class="input" style="resize:vertical">{!! $combinedBody !!}</textarea>
    </div>
</div>
...
<script>
    let cookiePolicyEditor;
    ClassicEditor.create(document.querySelector('#cookie_policy_body')).then(editor => {
        cookiePolicyEditor = editor;
    }).catch(err => console.error(err));

    document.querySelector('form').addEventListener('submit', function () {
        if (cookiePolicyEditor) {
            document.querySelector('#cookie_policy_body').value = cookiePolicyEditor.getData();
        }
    });
</script>
<style>
