...
<style>
    ...
    .image-preview { width:120px; height:120px; object-fit:cover; border-radius:0.75rem; border:1.5px solid #e5e7eb; display:none; margin-top:0.5rem; }
</style>
...
<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.75rem;margin-bottom:1.5rem">
    <div>
        ...
        <h2 style="font-size:1.5rem;font-weight:800;color:#111827;margin:0">Create Blog Post</h2>
        <p style="font-size:0.85rem;color:#9ca3af;margin-top:0.25rem">Draft and publish a new article</p>
    </div>
...
<div class="card">
    ...
    <div class="grid-3">
        ...
        <div>
            ...
            <input type="text" name="reading_time" value="{{ old('reading_time', '5 min read') }}" class="input" placeholder="e.g. 6 min read">
        </div>
        <div>
            <label class="label">Publish Date</label>
            <input type="datetime-local" name="published_at" value="{{ old('published_at', now()->format('Y-m-d\TH:i')) }}" class="input">
        </div>
    </div>
</div>
...
<div class="card">
    ...
    <div class="grid-2" style="margin-bottom:1rem">
        <div>
            ...
            <img id="preview" class="image-preview" alt="Preview Image">
        </div>
        <div style="display:flex;flex-direction:column;justify-content:center;gap:0.75rem">
            <label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer">
                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} style="width:1.15rem;height:1.15rem;accent-color:#6366f1">
                <span style="font-size:0.875rem;color:#4b5563;font-weight:600">⭐ Mark as Featured Post</span>
            </label>
            <label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', '1') ? 'checked' : '' }} style="width:1.15rem;height:1.15rem;accent-color:#6366f1">
...
<div class="card">
    ...
    <div>
        ...
        <textarea id="editor" name="content" rows="12" class="input" style="resize:vertical">{{ old('content') }}</textarea>
    </div>
</div>
